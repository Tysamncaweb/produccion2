...
class TestImportCsvEmployeeIncomeWh(TransactionCase):
    def setUp(self):
        ...
        self.period_id = ap_obj.find(self )
    ...
    def _create_ixwd(self,uid):
        values = dict(
            name='Importing CSV File',
            period_id=self.period_id[0],
            user_id=uid,
        )
        self.ixwd_id = self.ixwd_obj.create(values)
    ...
    def _create_rp(self):
        ...
        self.rp_id = self.rp_obj.create( values)
    ...
    def _create_eiw(self, filetype='csv'):
        ...
        self.eiw_id = self.eiw_obj.create(values)

    def _import_csv_xml(self, filetype='csv'):
        self._create_ixwd()
        self._create_eiw(filetype)
        ixwd_brw = self.ixwd_obj.browse(self.ixwd_id)
        ...
        self.eiw_obj.process_employee_income_wh(self.eiw_id,
                                                context=context)
        ixwd_brw = self.ixwd_obj.browse(self.ixwd_id)
