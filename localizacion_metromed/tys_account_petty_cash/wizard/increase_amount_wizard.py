...
class ChangeAmountPettyCashWizard(models.TransientModel):

    """
    Wizard that increase the amount petty cash.
    """
    _name = 'change.amount.petty.cash.wizard'
    _description = 'Change amount petty cash'

    increase_amount_petty_cash = fields.Monetary('amount increased')
    ...
    @api.onchange('increase_amount_petty_cash')
    def _get_amount_new(self):
        self.amount_new = self.amount + self.increase_amount_petty_cash
        #amount_disponible = self.env['account.petty.cash'].search([('code', '=', self.active_id)])
        #amount_disponible.write({'disponible': self.amount_new})
        return


    def validate_petty_cash_amount_open(self):
        if self.increase_amount_petty_cash <= 0:
    ...
    def validate_petty_cash(self,petc_ids):
        ...
        self.move_petty_cash_ = {
            'account_id': petty_cash.petty_cash_bank_id.default_debit_account_id.id,
            ...
            'name': petty_cash.name,
            'journal_id': petty_cash.petty_cash_journal_id.id,
            'credit': self.increase_amount_petty_cash,
            'debit': 0.0,
        }
        ...
        asiento['account_id'] = petty_cash.petty_cash_account_id.id
        asiento['credit'] = 0.0
        asiento['debit'] = self.increase_amount_petty_cash
        ...
        if move_line_id1 and move_line_id2:
            petty_cash.write({'petty_cash_status': 'validate',
                   ...
                   'petty_cash_amount_open': petty_cash.petty_cash_amount_open + self.increase_amount_petty_cash,
                   'disponible': petty_cash.petty_cash_amount_open + self.increase_amount_petty_cash,
                   'name': petty_cash.name})
